...
public class SedanDriverTest {
	...
	public void setup(){
		...
		drivers = new SedanDriver[4096];
		for(int i = 0; i<drivers.length ; i++){
			drivers[i] = new SedanDriver(rnd.nextLong()); //Use random seed for each driver
		}
	}
	
	@Test
	public void ensureShopTimeUpToLimits(){
		//Drivers should shop for 2-5 minutes (12-30 ticks)
	...
	public void ensureSpendingMoneyUpToLimits(){
		//Drivers should have £8.00-£16.00 to spend
		...
		//Ensure that min and max are equal to the limits
		assertEquals(new BigDecimal(8.00).setScale(2, RoundingMode.HALF_UP), minMoney);
		assertEquals(new BigDecimal(16.00).setScale(2, RoundingMode.HALF_UP), maxMoney);
	}
	
	@Test
	public void testTimeAtStation(){
		//Iterate through array of drivers
		for(SedanDriver driver : drivers){
	...
	public void checkWontShopOverTime(){
		//Driver should not shop if time in station is >= 10 minutes (60 ticks)
		
		//Iterate through drivers and simulate 60 - 560 ticks in station
		for(SedanDriver driver : drivers){
			for(int i = 0 ; i< 60 + rnd.nextInt(501) ; i++){
				driver.incrementTime();
			}
		}
	...
	public void checkWillShopWithProbabilityWithinTime(){
		//Driver should shop with 0.4 probability when time in station is < 10 minutes (0-59 ticks)
		
		//Iterate through drivers and simulate 0 - 59 ticks in station
		for(SedanDriver driver : drivers){
		...
		assertEquals(0.400, shopProbability, 0.01);
	}
